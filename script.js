...
$(window).on('load', () => {
	const regx = [ 
		/\d+(?:\d|,)*(?:\.\d+)?(?:-\d+(?:\.\d+)?)?\s*inch(?:es)\b/gi,
		/\d+(?:\d|,)*(?:\.\d+)?(?:-\d+(?:\.\d+)?)?\s*(?:foot|feet|ft)\b/gi,
		/\d+(?:\d|,)*(?:\.\d+)?(?:-\d+(?:\.\d+)?)?\s*mile(?:s)?\b/gi,
		/\d+(?:\d|,)*(?:\.\d+)?(?:-\d+(?:\.\d+)?)?\s*yard(?:s)?\b/gi,

		/\d+(?:\d|,)*(?:\.\d+)?(?:-\d+(?:\.\d+)?)?\s*(?:pound(?:s)?|lbs)\b/gi,
		/\d+(?:\d|,)*(?:\.\d+)?(?:-\d+(?:\.\d+)?)?\s*ton(?:s)?\b/gi,
		/\d+(?:\d|,)*(?:\.\d+)?(?:-\d+(?:\.\d+)?)?\s*(?:ounce(?:s)?|oz)\b/gi,
		/\d+(?:\d|,)*(?:\.\d+)?(?:-\d+(?:\.\d+)?)?\s*gal(?:lon(?:s)?)?\b/gi,

		/\d+(?:\d|,)*(?:\.\d+)?(?:-\d+(?:\.\d+)?)?\s*(?:(?:°\s*)?\s*(?:f|fahrenheit)|degrees)\b/gi,

		/\d+(?:\d|,)*(?:\.\d+)?(?:-\d+(?:\.\d+)?)?\s*mph\b/gi,
